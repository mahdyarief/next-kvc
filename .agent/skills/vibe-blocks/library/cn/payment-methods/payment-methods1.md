...
- **Objective**: Provide a clean, vertical list interface for managing saved payment methods (credit cards, PayPal).
- **Use Case**: Commonly used in user account settings or a dedicated "Wallet" page where users add, edit, or remove payment options.
- **Visual Style**: Clean, minimal, utilizing bordered cards in a vertical stack. Uses real-world payment provider logos for quick recognition. Inline forms for editing and adding new methods.
- **Interactivity**: Clicking a card selects it (via a radio group). A dropdown menu on each card offers "Edit" and "Remove" actions. Editing or adding a new method expands an inline form within the card layout.
