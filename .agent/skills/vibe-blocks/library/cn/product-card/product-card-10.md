...
- **Objective**: Display a product card with variant-specific thumbnail selection and action buttons revealed on hover.
- **Use Case**: Home decor or ceramics where variant selection is based on visual patterns or colors.
- **Visual Style**: Clean, modern card with vertical aspect ratio (3:4 approximately), rounded corners, and centered action buttons.
- **Interactivity**: 
    - Selecting a variant thumbnail updates the main product image.
    - Hovering over the card scales the main image slightly.
    - Action buttons ("Add to cart", "Quick View") fade and slide into view from the bottom on hover.
    - Status badges (Sale, Sold Out) appear in the top-left corner.
    - Tooltips for variant names on the thumbnails.

## Description
Product Card 10 is an elegant solution for products with visual variants. It displays thumbnails of each variant below the product price, allowing users to switch the main image by clicking a variant. The interaction design is centered around a "hover for actions" pattern, keeping the default state clean and focusing on the product imagery. It incorporates `useState` to manage selection and updates the main image dynamically.
