...
- **Objective**: Display a clean, detailed product card with a badge and price.
- **Use Case**: E-commerce listings, collections, or featured product sections.
- **Visual Style**: Clean card layout with a specific aspect ratio image, bold title, and prominent pricing (handling regular and sale prices).
- **Interactivity**: The entire card is a link with a subtle hover opacity effect.

## Description
Product Card 1 is a robust, standard e-commerce card. It uses Shadcn UI's `Card` component for structure and `AspectRatio` to maintain image consistency. It features a custom `Badge` with a configurable background color and handles sale pricing through a custom `Price` component.
