...
- **Objective**: Display a product card with size selection and inventory status indicators.
- **Use Case**: Clothing and apparel stores where size availability is key context.
- **Visual Style**: Clean vertical layout, high-contrast red for sale pricing, and a custom hover drawer/overlay for size selection.
- **Interactivity**: 
    - Desktop: Hover reveals a size selection overlay at the bottom of the image.
    - Mobile: Clicking the shopping bag icon opens a `Drawer` for size selection.
    - Tooltips for out-of-stock or low-stock status on desktop.
    - Automatic form submission upon size selection.

## Description
Product Card 7 is an advanced clothing card that solves the size-selection problem elegantly across devices. It handles multiple stock states (`IN_STOCK`, `LOW_STOCK`, `OUT_OF_STOCK`) with specific visual cues and tooltips. On desktop, the size options appear instantly on hover to reduce friction, while on mobile, it uses a bottom drawer to provide a comfortable touch interface.
