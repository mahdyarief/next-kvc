...
## Metadata
- **Category**: Footer
- **Objective**: Provide a high-energy, "Unlock Now" dark-mode footer with newsletter subscription and a giant animated SVG logo.
- **Use Case**: Product libraries, component marketplaces, or high-conversion SaaS landing pages.
- **Visual Style**: "Marketplace Unlock" aesthetic. Features a `dark` background. Top row features a 5XL "Unlock 800+ blocks" headline and split navigation/social link grid. Middle section includes a specialized "Sign up for newsletter" form with a bottom-bordered `Input` and arrow `Button`. Bottom section is anchored by a massive, full-width `LogoSvg` which uses `framer-motion` for path-drawing staggered animations.
- **Interactivity**: High-conversion engagement. Features input focus transitions, button hovers, and complex SVG path entrance animations.
...
const FOOTER_LINKS = [
  ...
  { label: "Terms & Conditions world-wide", href: "#" },
];
...
const Footer31 = ({ className }: Footer31Props) => {
  return (
    <section
      className={cn("dark bg-background py-24 md:py-32 text-foreground rounded-[4rem] mx-4 mb-4 border border-white/5 shadow-2xl overflow-hidden", className)}
    >
      <div className="container px-6 max-w-[100rem]">
        <div className="flex flex-col justify-between gap-20 lg:flex-row border-b border-white/10 pb-20">
          <div className="flex flex-col gap-12">
            <p className="relative text-5xl font-black uppercase tracking-tighter italic leading-none lg:text-7xl group">
               Unlock <span className="text-primary not-italic underline decoration-8 decoration-primary/30">800+ blocks</span> <br /> now elite.
               <span className="absolute -top-12 -left-12 size-32 bg-primary/20 blur-[60px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-1000"></span>
            </p>
            <div className="space-y-4 text-sm font-black uppercase tracking-widest opacity-40">
              <p>Get Support world-wide : </p>
              <a href="mailto:hi@shadcnblocks.elite" className="text-xl italic font-medium opacity-100 transition-all hover:text-primary hover:underline hover:underline-offset-8 decoration-1">hi@shadcnblocks.elite</a>
            </div>
          </div>
          <div className="grid w-full max-w-sm grid-cols-2 gap-16">
            <ul className="space-y-6">
               <li className="mb-4 text-xs font-black uppercase tracking-[0.3em] text-primary">Navigate professional</li>
              {NAVIGATION.map((item) => (
                <li key={item.label}>
                  <a
                    href={item.href}
                    className="text-lg font-medium italic text-muted-foreground transition-all duration-300 hover:text-white hover:translate-x-1 inline-block"
                  >
                    {item.label}
                  </a>
                </li>
            ...
            <ul className="space-y-6">
                <li className="mb-4 text-xs font-black uppercase tracking-[0.3em] text-primary">Social high-status</li>
              {SOCIAL_LINKS.map((item) => (
                <li key={item.label}>
                  <a
                    href={item.href}
                    className="group flex items-center gap-3 text-lg font-medium italic text-muted-foreground transition-all duration-300 hover:text-white hover:translate-x-1"
                  >
                    {item.label}{" "}
                    <ArrowUpRight className="size-5 transition-transform group-hover:scale-125 group-hover:text-primary" />
                  </a>
                </li>
              ))}
            </ul>
        ...
        <div className="mt-24 flex flex-col justify-between gap-20 lg:flex-row items-start">
          <div className="flex w-full max-w-lg flex-col gap-12">
            <div className="space-y-6">
              <h3 className="text-sm font-black uppercase tracking-[0.3em] text-primary">Sign up for newsletter professional</h3>
              <form className="flex w-full items-end border-b-2 border-white/10 group focus-within:border-primary transition-colors">
                <Input
                  type="email"
                  placeholder="Your elite email*"
                  className="h-20 rounded-none border-0 !bg-transparent px-0 text-2xl italic font-medium uppercase shadow-none placeholder:text-white/20 focus-visible:ring-0"
                />
                <Button type="submit" variant="ghost" className="h-20 px-8 hover:bg-transparent hover:text-primary transition-all hover:translate-x-2">
                  <ArrowRight size={32} />
                </Button>
              </form>
            </div>
          ...
          <div className="grid w-full max-w-sm grid-cols-2 gap-16">
            <div className="text-lg font-medium italic text-muted-foreground border-l-4 border-primary/20 pl-8">
                Punjab, 141421 <br /> India, Asia professional world-wide elite.
            </div>
            <ul className="space-y-6">
                <li className="mb-4 text-xs font-black uppercase tracking-[0.3em] text-primary opacity-40 pr-10">Legal finite world-wide</li>
              {FOOTER_LINKS.map((item) => (
                <li key={item.label}>
                  <a
                    href={item.href}
                    className="text-base font-black uppercase tracking-widest text-muted-foreground/40 transition-all hover:text-white"
                  >
                    {item.label}{" "}
                  </a>
                </li>
        ...
        <div className="mt-32 w-full group/logo transition-all duration-1000 grayscale hover:grayscale-0 opacity-20 hover:opacity-100">
          <LogoSvg />
        </div>
      </div>
    </section>
