...
- **Objective**: Provide a streamlined profile management interface focused on core identity.
- **Use Case**: User settings pages, dashboard profile tabs, and basic account setup flows.
- **Visual Style**: Compact card-based layout with a prominent circular avatar upload component and clear field hierarchy.
- **Interactivity**: Single-file avatar upload with real-time preview, hovering camera icon for upload trigger, input validation for email, and multiline bio support.

## Description
Settings Profile 1 is a focused component for updating basic user information. It features a clean Shadcn UI Card that houses an interactive avatar uploader alongside standard form fields for name, username, email, and biography. The design prioritizes ease of use and visual feedback for file uploads.
