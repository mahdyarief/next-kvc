...
## Metadata
- **Category**: Blog
- **Objective**: Display a sidebar-filtered blog post listing with category navigation.
- **Use Case**: Blog archives, news sections, and content hubs requiring sidebar category filtering.
- **Visual Style**: Two-column layout with sticky sidebar navigation and grid-based post display with hover image effects.
- **Interactivity**: Static category navigation links with hover effects on post cards.

## Description
A blog listing component featuring a two-column layout with a sticky sidebar containing category navigation and a grid display of blog posts. Each post includes a title, publication date, and features a hover effect that reveals the post image with a smooth clip-path animation. The sidebar includes category filters (All, Design, Development, Marketing) and a decorative FileText icon. The layout is responsive and includes smooth transitions for the image reveal effect.
