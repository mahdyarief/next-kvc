...
## Metadata
- **Category**: Blog
- **Objective**: Display a filterable blog post listing with category selection.
- **Use Case**: Blog archives, news sections, and content hubs requiring category filtering.
- **Visual Style**: Grid layout with dropdown filters and card-based post display.
- **Interactivity**: Category filter dropdown with hover effects on post cards.

## Description
A blog listing component featuring a category filter dropdown and a responsive grid layout for displaying blog posts. Each post includes an image, title, summary, and category label. The component uses a select dropdown for filtering posts by category, and includes hover effects that scale the post images. The layout is fully responsive, adapting from a single column on mobile to three columns on large screens.
