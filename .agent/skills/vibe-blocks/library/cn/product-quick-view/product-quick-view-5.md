...
- **Objective**: Provide a comprehensive and professional product overview modal that replicates the full PDP (Product Detail Page) experience in a compact `Dialog`.
- **Use Case**: High-end electronics, appliances, or complex products where technical specifications and detailed features are necessary for conversion.
- **Visual Style**: Industrial and Technical. Uses a wide, dual-pane `Dialog` with a focus on information density and high-quality media presentation. Features sophisticated stock status indicators and integrated service promises.
- **Interactivity**: 
    - Advanced PhotoSwipe Integration: Replaces the standard carousel viewing with a professional `PhotoSwipeLightbox`. Includes custom UI registration for a synchronized indicator, close button, and navigation arrows.
    - Synchronized Thumbnails: Features a scrolling thumbnail strip with fade-out mask effects that maintains sync with the main carousel and lightbox.
    - Dynamic Stock Messaging: Provides real-time stock feedback with status-driven colors (`Ready to ship`, `Selling fast`, `Out of Stock`) and specific "Preorder" quantities.
    - Technical Accordions: Uses an `Accordion` section to organize long-form content, technical features lists, and shipping policies without cluttering the main view.
    - Quantity Stepper: Includes a dedicated `QuantityInput` for bulk selection.
    - Social Sharing: Integrates branded social media icons (`SimpleIcons`) for easy sharing directly from the modal.
    - Modal Responsiveness: Optimized for mobile with a "bottom-sheet" style transition and full-screen height optimization.

## Description
Product Quick View 5 is the "Pro" version of the quick view interface. It is significantly more feature-dense than standard modals, providing everything from ratings and service icons to technical accordions and social links. The core innovation is the deep integration between `Carousel` and `PhotoSwipe`, offering users a professional-grade image gallery experience. It is designed to provide enough information for a user to complete a high-value purchase without ever navigating to the full product page.
