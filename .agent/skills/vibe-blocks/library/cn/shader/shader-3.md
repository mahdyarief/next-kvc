...
## Metadata
- **Category**: Shader
- **Objective**: Provide an interactive, hyper-realistic "Glass Refraction" background where a glowing orb follows the user's cursor behind a series of fluted glass rods.
- **Use Case**: Premium SaaS landing pages, digital design agency portfolios, or any site that demands an extremely high level of interactive visual fidelity.
- **Visual Style**: Photorealistic "Fluted Glass" architecture. Rendered entirely via maths (`fract`, `sin`, `length`) in GLSL, bypassing the need for heavy 3D models. It calculates physical light phenomena including Fresnel effects, caustic lighting, and refractive deformation index based on the `u_mouse` uniform tracking.
- **Interactivity**: High-engagement cursor tracking via `window.addEventListener('mousemove')` that lerps (`lerpFactor = 0.025`) the `u_mouse` uniform for a smooth, trailing response. The background actively reacts to the user's presence, bending the light of the "orb" through the static grid.

## Description
Shader 3 is the "Refractive Orbs" component. It prioritizes the "Pinnacle Premium" brand archetype by deploying visual effects normally reserved for pre-rendered video directly into a real-time DOM element. By simulating physical glass attributes over a moving, user-controlled light source, it creates an immediate sense of tactile quality. It converts a standard hero section into a playground, dramatically increasing time-on-page as users naturally play with the refraction mechanics before scrolling to read the content.
