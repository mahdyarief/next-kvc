...
## Metadata
- **Category**: Shader
- **Objective**: Interactive grid with morphing shapes
- **Use Case**: Interactive backgrounds, creative coding demos, artistic displays
- **Visual Style**: Grid-based system with shapes that morph from squares to crosses to lines
- **Interactivity**: Mouse movement creates animated shapes that age and morph over time

## Description
A Three.js shader-based interactive grid with morphing shapes. Uses advanced ping-pong rendering techniques with frame buffer objects (FBO) for complex animations. Features a grid system where mouse interaction creates shapes that appear and then morph through three stages: square → cross → line, before disappearing. The shader uses two-pass rendering with a simulation pass and a display pass, creating smooth, organic morphing animations. Perfect for creating interactive, artistic backgrounds for websites, creative coding projects, or visual experiences that respond to user interaction with sophisticated animation techniques.
