...
- **Objective**: Render a hierarchical, deeply nested folder/file structure with collapsible nodes and interactive selections.
- **Use Case**: Best used in file explorers, project sidebars, complex navigation menus, or organizational charts where deep hierarchy needs to be visualized cleanly.
- **Visual Style**: Clean, modular sidebar aesthetic utilizing `lucide-react` icons (`Folder`, `FolderOpen`, `File`, `ChevronRight`). Features precise, deeply calculated indentation blocks (`TreeLines`) rendering vertical and horizontal connecting borders to delineate parent/child nesting relationships visually. Supports hover/active states via `bg-accent` color variants.
- **Interactivity**: Fully controlled or uncontrolled state handling for expansion and selection (`useTree`). Driven heavily by `motion/react` (`AnimatePresence`) for smooth, staggered unfolding animations of nested children. Supports multi-selection via `Ctrl/Cmd` click.
