...
- **Objective**: Display a highly polished, interactive 3D model of a credit card for payment interfaces or checkout flows.
- **Use Case**: Best used in e-commerce checkout forms, billing portals, or settings pages to give users a visual anchor of the card they are using or adding.
- **Visual Style**: Incorporates sleek 3D perspective variables via CSS classes (`perspective-distant`, `-rotate-y-180`, `transform-3d`). It dynamically constructs a beautiful SVG card chip (`CreditCardChip`) with a custom gold linear gradient, and perfectly aligns typography (e.g., monospace numbers mapped to `CreditCardNumber`).
- **Interactivity**: The component boasts 3D flip animation functionality (`CreditCardFlipper`) that can trigger on hover or click. A custom hook (`useSupportsHover`) checks if the user's device supports hover states to decide whether touching or hovering trips the CSS `transform: rotateY()` animation, enhancing accessibility for mobile/touch devices.
