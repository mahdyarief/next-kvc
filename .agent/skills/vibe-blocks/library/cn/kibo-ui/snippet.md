...
- **Objective**: Display small, strictly formatted code snippets (such as CLI installation commands) inside a compact tabbed interface.
- **Use Case**: Best used in developer documentation when showing package installation instructions across different package managers (e.g., `npm`, `yarn`, `pnpm`, `bun`). 
- **Visual Style**: Clean, compact wrapper based on Shadcn's underlying `Tabs` primitive. Includes a clean subtle border and a secondary background for the header.
- **Interactivity**: Fully interactive tabs interface. Includes an embedded `SnippetCopyButton` which accesses the OS clipboard via `navigator.clipboard.writeText`, providing instant user feedback by toggling a `CheckIcon` for 2 seconds upon success, which only becomes visible on hover of the specific line snippet.
