...
- **Objective**: Display a large volume of logos without consuming excessive vertical screen real estate.
- **Use Case**: Homepages with 8+ integration partners or prominent enterprise clients where scrolling through a large static grid would be tedious.
- **Visual Style**: Uses the Shadcn UI `Carousel` primitive. Employs CSS positioning to overlay linear gradient masks (`bg-linear-to-r from-background to-transparent`) on the left and right edges, creating a seamless fade-out effect as logos exit the viewport.
- **Interactivity**: Infinite automated horizontal scrolling powered by the `embla-carousel-auto-scroll` plugin (`playOnInit: true`, `loop: true`).
