...
## Metadata
- **Category**: Sidebar
- **Objective**: Sidebar with dropdown-style submenus.
- **Use Case**: Applications where nested navigation should appear as floating dropdown menus rather than inline expanding accordions, saving vertical space.
- **Visual Style**: Clean, standard sidebar aesthetics with floating submenu dropdowns.
- **Interactivity**: Clicking on menu items with children triggers a contextually positioned dropdown menu containing the sub-navigation items.

## Description
A space-efficient navigation pattern that uses standard floating dropdowns for hierarchical depth. This approach is ideal for sidebars with many top-level categories where vertical expansion might cause excessive scrolling.
...
// Nav item with dropdown menu for children
...
const Sidebar15 = ({ className }: Sidebar15Props) => {
  return (
