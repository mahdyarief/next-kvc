...
## Metadata
- **Category**: Sidebar
- **Objective**: Space-saving collapsible sidebar that shrinks to an icon-only view.
- **Use Case**: Applications with limited screen space or those that prioritize the main content area while keeping navigation accessible.
- **Visual Style**: Dynamic sidebar that transitions between full and icon-only states.
- **Interactivity**: Automatic collapsing to icons with tooltips for accessibility and smooth layout transitions.

## Description
A highly efficient sidebar that supports an icon-only mode, maximizing real estate for the primary application content without sacrificing navigation utility.
...
interface Sidebar8Props {
  className?: string;
}

const Sidebar8 = ({ className }: Sidebar8Props) => {
  return (
