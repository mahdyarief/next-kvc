...
## Metadata
- **Category**: Sidebar
- **Objective**: Dual-sidebar layout with both left and right sidebars.
- **Use Case**: Feature-rich dashboards where the left sidebar handles primary navigation and the right sidebar displays context-specific metrics or technical details.
- **Visual Style**: Balanced layout with two sidebars framing the central content area.
- **Interactivity**: Left sidebar for main navigation; right sidebar for dense technical metrics.

## Description
A comprehensive navigation system featuring dual sidebars to categorize different areas of functionality: primary application navigation on the left, and technical metrics or secondary controls on the right.
...
// Dashboard sections for right sidebar - dense technical menu
      ...
      <SidebarContent className="gap-0">
        ...
        {dashboardSections.map((section) => (
          <SidebarGroup key={section.title} className="py-2">
            <SidebarGroupLabel className="px-3 text-[10px] tracking-wider text-muted-foreground/70 uppercase">
              {section.title}
            </SidebarGroupLabel>
            <SidebarGroupContent>
              <div className="flex flex-col">
                {section.items.map((item) => (
                  <button
                    key={item.label}
                    className={cn(
                      "px-3 py-1 text-left text-xs transition-colors hover:bg-muted/50",
                      item.isActive
...
interface Sidebar12Props {
  className?: string;
}

const Sidebar12 = ({ className }: Sidebar12Props) => {
  return (
