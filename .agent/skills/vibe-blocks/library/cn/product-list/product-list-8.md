...
- **Objective**: Implement a "Micro-PDP" (Product Detail Page) card that consolidates all shopping decisions—size, color, and quantity—into a single, high-density component within the list view.
- **Use Case**: Quick-order catalogs, bulk purchasing interfaces, or fashion retailers where users are likely to buy multiple variants of the same item.
- **Visual Style**: Industrial and Brutalist. Features a continuous "mesh" grid layout achieved with `gap-px` and card-level ring borders. High focus on visual texture with hover-triggered image swaps (`images: [Image, Image]`), numeric ratings, and clear discount visualization.
- **Interactivity**: 
    - Hover Image Reveal: Features a dual-image system that switches from a front view to an alternate or detail view on desktop hover, providing instant secondary visual information.
    - Collapsible Configuration Drawer: Clicking the product action button (`ShoppingBag`) slides in an absolute configuration panel over the entire card, effectively turning the list item into a specialized mini-interface.
    - Comprehensive Configuration Form: Uses `react-hook-form` and `zod` to manage `size` (text chips), `color` (swatches), and `quantity` (stepper).
    - Intelligent Stock Feedback: Color swatches include a strike-through effect for out-of-stock items, and the "Add to cart" button remains intelligently disabled until a valid variant is selected.
    - Quantity Stepper: A custom-built `Quantity` input with `-`/`+` buttons that enforces `min` and `max` constraints at the field level.
    - Scroll-Optimized Panel: The configuration overlay is wrapped in a `ScrollArea` to maintain usability across varying numbers of product options.

## Description
Product List 8 is a power-user component designed for maximum efficiency. It eliminates the need for users to bounce between the gallery and the detail page by bringing the "Add to Cart" form directly to the product grid. The design is intentionally minimalist and high-reach, prioritizing speed and clarity. By utilizing a "collapsible over card" pattern, it preserves the grid's rhythm while unlocking deep interactivity on demand. This component is the perfect choice for stores with repetitive shopping patterns or highly configured products like multi-size/multi-color apparel.
