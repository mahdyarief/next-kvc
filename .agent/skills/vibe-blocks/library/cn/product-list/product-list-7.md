...
- **Objective**: Deliver a personalized recommendation grid ("Suggestions just for you") that maximizes conversion through predictive variant selection and cross-device interaction patterns.
- **Use Case**: Post-purchase cross-sell, "You may also like" sections, or AI-driven personalized landing pages.
- **Visual Style**: Premium and data-rich. Uses a tight `auto-fit` grid with large imagery (`0.78` AspectRatio). Employs high-contrast red accents (`text-red-600`) for sales and percentage badges to drive urgency. Features elegant circular variants and subtle `Tooltip` feedback for stock levels.
- **Interactivity**: 
    - Device-Specific Checkout Flows: 
        - Desktop: A seamless hover overlay revealing size options for frictionless adding. 
        - Mobile: A dedicated `Drawer` (bottom sheet) triggered by a `ShoppingBag` icon, providing a high-reachability touch interface for variant selection.
    - Low-Stock Urgency: Integrated "LOW_STOCK" indicators ("Only a few left!") within size swatches and tooltips to stimulate immediate purchase.
    - One-Tap Selection: Selecting a size variant through the `RadioGroup` automatically triggers a form submission, reducing clicks-to-cart.
    - High-Value Utility: Features a secondary `Heart` wishlist button (desktop) and distinct product badging for discount visualization.

## Description
Product List 7 is an ultra-conversion-optimized component designed to feel like a personal shopping assistant. It recognizes the fundamental differences between desktop (mouse-driven) and mobile (thumb-driven) interactions by offering a unique "hover to expand" desktop view and a "bottom sheet drawer" mobile experience. The technical foundation is robust, utilizing `react-hook-form` to manage individual card states and providing helpful tooltips for out-of-stock or low-stock items. It's the ideal choice for retailers looking to boost their Average Order Value (AOV) via smart recommendations.
