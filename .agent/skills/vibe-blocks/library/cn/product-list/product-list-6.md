...
- **Objective**: Implement a high-performance "Most Popular" product carousel with integrated state management for color variants and quick-purchase actions.
- **Use Case**: Core homepage discovery, "Trending Now" sections, or any high-traffic retail environment where rapid decision-making is key.
- **Visual Style**: Tech-Modern and Fashion-forward. Features a minimalist, borderless layout with high visual feedback. Scalable imagery, hover-reveal buttons, and custom radio-group color swatches denotated by clean circular indicators.
- **Interactivity**: 
    - Real-Time Variant Sync: Integrated `react-hook-form` ensures that selecting a color swatch instantly updates the product image and internal form state without a page reload.
    - Quad-State Hover Logic: 
        - Zoom: Product images scale up (`group-hover:scale-110`) for a detailed look.
        - CTA Reveal: "Quick Add" button appears prominently on the image area.
        - Option Reveal: Color swatches fade in to allow variant selection before clicking.
        - Navigation Reveal: Pricing is swapped for "Shop Now" and "Quick View" action links on desktop hover.
    - Responsive Carousel Navigation: Uses separate `CarouselPrevious`/`Next` controls for desktop and custom animated progress bars/bullets for mobile tactile feedback.
    - Stock Check: Conditional rendering for "Sold out" vs "Sale" statuses with distinct visual styling.

## Description
Product List 6 is a flagship carousel component that bridges the gap between a list and a product detail page. By embedding variant selection and "Quick Add" functionality directly into the card, it significantly increases the efficiency of the shopping journey. It utilizes advanced React patterns (controlled forms per card) to ensure that the user's intent is captured accurately. The component is highly responsive, utilizing CSS grid basis (`sm:basis-1/2 xl:basis-1/4`) to provide the optimal balance of image size and item density across all devices.
