...
- **Objective**: Create a high-impact lifestyle storefront that alternates large promotional "Hero" cards with dense product grids.
- **Use Case**: Department store landing pages, seasonal lookbooks, or home-decor catalogs where high-quality lifestyle imagery is used to categorize product subsets.
- **Visual Style**: Luxurious and Refined. Features alternating flex rows (`lg:even:flex-row-reverse`) to create a "Z-pattern" scanning flow. Product cards use rounded-xl corners and a minimalist layout that hides complex controls until hovered.
- **Interactivity**: 
    - Alternating Layout: Automatically mirrors the layout for even-numbered sections to prevent visual fatigue and maintain user engagement.
    - Hover Action Cluster: "Add to cart" and "Quick View" buttons appear in a vertical stack over the product image on desktop hover, using a sliding fade-in transition.
    - Lifestyle Promotion Cards: Uses large `background-image` cards with overlay text and primary CTAs to drive users to collection-specific landing pages.
    - Intelligent Badging: Highlights "Sold Out" items with a standard badge and "Sale" items with a destructive-variant badge.
    - Zoom Hover Effect: Product images subtly scale up (`group-hover:scale-110`) to provide immediate feedback.

## Description
Product List 10 is designed for storytelling at scale. Unlike standard grids, it anchors every product subset with a high-conversion "Featured Promotion" block. This structure allows retailers to contextualize their products within a lifestyle setting before the user begins individual browsing. The hover-based interactivity on the product cards keeps the initial view clean and focused on visual aesthetics, making it a perfect fit for premium brands.
