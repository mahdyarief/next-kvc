...
## Metadata
- **Category**: Pricing
- **Objective**: Display an extremely scalable feature matrix built for high performance on both mobile (via single-column collapsible views) and desktop (via standard grid matrices).
- **Use Case**: Best for feature-heavy services where explaining *what is included* takes precedence over the *price tags* themselves. 
- **Visual Style**: A minimalist, tabular layout that heavily relies on grid CSS for alignment without using native HTML `table` elements. On desktop (`hidden md:grid md:grid-cols-4`), it separates the feature labels from the tier columns visually. On mobile, it uses a vertically stacked layout where only one tier is visible at a time. Highlighted tiers receive a subtle `bg-muted border rounded-xl` treatment.
- **Interactivity**: Functionally identical mobile behavior to Pricing 23 (`Collapsible` + Dropdown selection to swap which tier is being viewed on mobile). Provides a highly optimized UX for reading complex feature sets on small screens.
