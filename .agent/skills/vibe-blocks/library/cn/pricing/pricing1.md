...
## Metadata
- **Category**: Pricing
- **Objective**: Display a concise, four-column grid comparing pricing tiers with consistent card heights.
- **Use Case**: Best for SaaS or service platforms offering multiple escalating subscription tiers (e.g., Free, Pro, Premium, Enterprise).
- **Visual Style**: Clean four-column grid (`lg:grid-cols-4`, `lg:divide-x`). Highlighting the "Pro" tier with a subtle background (`bg-muted`). The cards employ a fixed-height top section (`h-[360px] flex-none`) to perfectly align features across the horizontal axis beneath them.
- **Interactivity**: Primarily layout-driven, with interactive buttons for purchasing or contacting sales. Designed with clear separation between pricing details and feature lists.
