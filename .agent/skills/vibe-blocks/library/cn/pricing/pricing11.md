...
## Metadata
- **Category**: Pricing
- **Objective**: Provide a highly detailed, two-part pricing structure: a sticky top summary row and a comprehensive sliding/collapsible feature matrix below.
- **Use Case**: Best for deeply complex SaaS products (like AWS, Vercel, or major CRMs) where users need to compare dozens of features across multiple tiers without losing track of which column represents which plan as they scroll.
- **Visual Style**: Complex, data-heavy layout. Uses a top section for plan summaries (`Tabs` for mobile billing cycle toggle, sticky positioning on desktop using `lg:sticky lg:top-16`). The bottom section uses definition lists (`dl`, `dt`, `dd`) to align features across the grid horizontally. Employs `Tooltip` for detailed feature explanations and `Badge` tags for add-on features. On mobile, the matrix collapses into segmented `Collapsible` accordions.
- **Interactivity**: Extremely interactive. Includes a `Tabs` component to toggle billing cycles. Employs `Tooltip` extensively for hovering over feature names (`Info` icon). Uses Shadcn UI's `Collapsible` component to handle the massive data matrix cleanly on mobile devices, ensuring the UI remains usable on small screens.
