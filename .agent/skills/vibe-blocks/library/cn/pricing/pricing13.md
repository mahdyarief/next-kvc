...
## Metadata
- **Category**: Pricing
- **Objective**: Display a clean grid of specific add-on or modular pricing options beneath a descriptive header.
- **Use Case**: Intended as a supplementary section, perhaps on a full pricing page, highlighting extra services, modular packages, or distinct a-la-carte upgrades.
- **Visual Style**: Clean, modular structure using a main wrapper (`rounded-xl border border-border`). The top half holds the section title, description, and an illustration. The bottom half contains a flexible grid (`flex-col xl:flex-row`) of add-on option cards, each highlighted with `bg-accent` and a clean separation of description and pricing.
