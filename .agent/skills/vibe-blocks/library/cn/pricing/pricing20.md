...
## Metadata
- **Category**: Pricing
- **Objective**: Display a stylish, modern three-tier pricing grid with a focused design language intended for a specific brand look.
- **Use Case**: Best for SaaS startups and creative agencies that want a slightly more opinionated, branded typography and spacing approach than a generic UI structure.
- **Visual Style**: Features custom typography sizing (`text-[3.5rem] tracking-[-3.92px]`), muted foreground secondary texts, and clear border grouping (`border-x`, `border-y`). Uses icons (`Rocket`, `Briefcase`, `Building`) for each tier. Employs Shadcn UI `Button`, and uses `BadgeCheck` for the feature lists instead of a standard `Check`. Includes decorative top and bottom spacer sections.
