...
## Metadata
- **Category**: Pricing
- **Objective**: Display a comprehensive feature matrix comparing multiple pricing tiers, with responsive adaptations for mobile and desktop.
- **Use Case**: Best for feature-heavy B2B products or enterprise software where users need a deep, categorized comparison of what each tier offers.
- **Visual Style**: Complex and highly responsive. On desktop, it presents a traditional, dense feature matrix using a standard HTML `table` with `Tooltip` elements for additional feature context. On mobile, the matrix collapses into individual `Card` elements for each tier, where features are listed below the pricing header in a vertical stack.
- **Interactivity**: Includes a billing toggle (`Switch`) to switch between annual and monthly pricing. Employs Shadcn UI `Tooltip` components to provide definitions or context for specific features within the table.
