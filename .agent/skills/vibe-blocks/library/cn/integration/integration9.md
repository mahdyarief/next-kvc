...
- **Objective**: Display a clean, multi-column inventory of available platform integrations.
- **Use Case**: Best used as the primary content of an "Integrations" directory page where users need to browse specific details of each tool.
- **Visual Style**: Clean minimalist design. Uses a structured 4-column grid (`lg:grid-cols-4`) of vertical cards. Each card features a small, centered icon inside a `muted` background square, followed by a bold title and a small, high-density description (`text-xs`).
- **Interactivity**: Subtle hover feedback. Cards transition from a shallow shadow (`shadow-sm`) to a deeper interaction state (`shadow-md`) on hover, creating a clickable interface feel.
