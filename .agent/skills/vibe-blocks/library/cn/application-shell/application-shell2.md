...
- **Objective**: Collapsible sidebar with icon-only mode
- **Use Case**: Compact navigation with hover tooltips
- **Visual Style**: Icon-based sidebar that expands on hover
- **Interactivity**: Collapsible sidebar with tooltips and user dropdown

## Description
A compact application shell featuring a collapsible sidebar that can switch between icon-only and expanded modes. Includes navigation groups with collapsible sections and a user profile dropdown.
...
export function ApplicationShell2({ className }: ApplicationShell2Props) {
  return (
