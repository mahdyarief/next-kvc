...
- **Objective**: Create a transparent, scroll-responsive navigation bar with an image-rich mega menu.
- **Use Case**: Marketing landing pages or media-rich sites where navigation blends into a hero section initially and reveals an opaque background upon scroll.
- **Visual Style**: The desktop mega menu uniquely features a large, prominent preview image on the left that changes dynamically when hovering over submenu links on the right. 
- **Interactivity**: Features a scroll event listener to toggle `bg-background` and `bg-transparent` classes at a predefined threshold (300px). Desktop dropdown menu uses custom DOM refs (`useRef`) to actively swap out the preview image side-panel without costly React re-renders. Uses `Sheet` for the mobile menu.
