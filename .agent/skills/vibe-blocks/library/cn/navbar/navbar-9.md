...
- **Objective**: Deliver a sleek, dark-themed (or invertible) navigation menu with a GitHub star counter.
- **Use Case**: Developer tools, open-source projects, or modern SaaS platforms needing a tech-centric, compact header.
- **Visual Style**: Uses custom `lucide-react` icons decorated with specific hex strokes for branding individual links. Keeps dropdown menus compact rather than expansive.
- **Interactivity**: Exposes a `GithubStars` component that dynamically fetches and renders a given repository's star count. Relies on Radix UI's `NavigationMenu` (with viewport set to `false`) for rapid tooltip-like submenu reveals. Uses an `Accordion` inside a full-screen `Sheet` for the mobile view.
