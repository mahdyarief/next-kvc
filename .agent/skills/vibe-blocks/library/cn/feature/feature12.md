...
## Metadata
- **Category**: Feature
- **Objective**: Scrollable feature carousel with progress bar and icon cards.
- **Use Case**: SaaS product feature showcases, platform capability listings, or onboarding section highlights.
- **Visual Style**: "Progress Carousel" aesthetic. Centered badge + heading. Below, a `Carousel` (Shadcn) shows 1/3 of items on desktop, 1/2 on tablet. Above the carousel: left-aligned "Rules" label and right-aligned `Progress` bar (`01 ─── 0N`) with prev/next controls. Each card has a circular `bg-accent` icon badge, title, and description.
- **Interactivity**: Scroll-driven progress. The `Progress` bar value updates live on carousel scroll using `api.on("scroll", { scrollProgress })`. Prev/Next controls are rendered `static` (not floated).
