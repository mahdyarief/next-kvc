...
## Metadata
- **Category**: Feature
- **Objective**: Advanced carousel synced with an interactive accordion-style feature list.
- **Use Case**: Deep feature tours on landing pages, complex product walkthroughs, or interactive narrative explorers.
- **Visual Style**: "Accordion-Carousel Sync" aesthetic. `flex flex-col md:flex-row-reverse` layout. Right (or top mobile): `aspect-5/6` vertical-format image carousel with `rounded-3xl` container. Left: interactive `ul` list where items expand like accordions when selected. Each list item: title + circular icon button trigger, expanding to show description and link.
- **Interactivity**: Fully synced `selection` state between the list and the `Embla` carousel. List items expand vertically via `group-data-open:block`. Triggers rotate (`group-data-open:rotate-180`) and change color on hover/selection.
