...
## Metadata
- **Category**: Feature
- **Objective**: Advanced carousel synced with an interactive accordion-style feature list (Carousel Left).
- **Use Case**: Narrative-driven landing pages, interactive product explorers, or visual storytelling sections.
- **Visual Style**: "Accordion-Carousel Sync (Mirror)" aesthetic. `flex flex-col md:flex-row` layout. Left side: `aspect-5/6` vertical-format image carousel. Right side: interactive `ul` list with triggers that expand vertically (`data-open:block`) to show description and link. Triggers feature a circular icon button with rotation animation.
- **Interactivity**: Fully synced state between triggers and `Embla` carousel.
