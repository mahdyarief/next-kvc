...
## Metadata
- **Category**: Feature
- **Objective**: Professional feature grid with a side-aligned header sticky/intro column.
- **Use Case**: Detailed capability directories, product landing page feature grids, or corporate service brochures.
- **Visual Style**: "Side-Header Feature Grid" aesthetic. Left column: constrained `lg:max-w-sm` section with bold `text-4xl` header, description, and link button. Right side: `md:grid-cols-2` grid of large rounded-border cards. Each card: `aspect-16/9` image on top, followed by a high-padding content area (`lg:px-10 lg:py-12`) with bold headings.
- **Interactivity**: Images feature an opacity hover transition (`hover:opacity-80`). Link button features an animated arrow icon.
