...
## Metadata
- **Category**: Feature
- **Objective**: Six-feature centered grid with large circular icon badges, centered heading, and CTA button.
- **Use Case**: SaaS product feature pages, component library showcases, or developer tool capability overviews.
- **Visual Style**: "Centered Icon Grid" aesthetic. Centered `max-w-3xl lg:text-5xl` heading. Six items in `md:grid-cols-2 lg:grid-cols-3`. Each feature: large `size-16 rounded-full bg-accent` icon badge, `text-xl font-semibold` title, and muted description. Large centered `size="lg"` button footer.
- **Interactivity**: Static. Prop-configurable via `title`, `features`, `buttonText`, `buttonUrl`.
