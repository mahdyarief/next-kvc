...
## Metadata
- **Category**: Feature
- **Objective**: Multi-section feature table with title headings and 3-column checklist grids separated by full-width dividers.
- **Use Case**: Product overview pages, SaaS feature tables, or platform capability breakdowns organized by category.
- **Visual Style**: "Divided Feature Table" aesthetic. Centered badge + heading + description. Three sections each preceded by a `Separator`. Each section: `lg:grid-cols-4` layout with a title on the left and a `col-span-3 lg:grid-cols-3` checklist grid of 9 items. Clean, document-like structure optimized for information density.
