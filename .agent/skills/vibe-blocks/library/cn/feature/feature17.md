...
## Metadata
- **Category**: Feature
- **Objective**: Six-feature list grid with secondary badge label, heading, and a centered CTA button footer.
- **Use Case**: SaaS feature comparison pages, product capability listings, or "More Features" showcase sections.
- **Visual Style**: "Feature Prop List" aesthetic. Left-aligned secondary `Badge` + large `md:text-5xl` heading. Below: six features in a `md:grid-cols-2` grid. Each feature: circular `bg-accent` icon badge (inline-flex on mobile, block on desktop) + title + description. Footer has a centered `size="lg"` primary button.
- **Interactivity**: Static layout. Fully configurable via props (label, title, features array, buttonText, buttonUrl).
