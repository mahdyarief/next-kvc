...
- **Objective**: Render a meticulously detailed, vector-based Safari macOS window frame to encase application screenshots or screen recordings.
- **Use Case**: Crucial for primary B2B heroic sections or major feature demonstrations, instantly elevating a flat screenshot into a premium app showcase.
- **Visual Style**: Heavy SVG architecture. Recreates native macOS UI elements, including the classic 'traffic light' window controls (three colored circles on the left), a deeply stylized URL address bar sporting a subtle drop shadow/glow (`mix-blend-luminosity`), and complex paths to render standard browser navigation icons (back/forward, extensions, etc.) depending on the chosen `mode` ("default" vs "simple").
- **Interactivity**: Primarily presentational. Permits embedded silent video looping (`videoSrc`) identically to the iPhone mockup. Offers an explicit `url` prop to print dynamic text directly onto the SVG canvas representing the fake address bar context.
