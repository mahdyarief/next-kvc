...
- **Objective**: Use a minimalist, item-centric list format inside plain cards, emphasizing individual product statuses within a larger order.
- **Use Case**: Works well for marketplaces or multi-shipment orders where individual items within the same order may arrive on different dates.
- **Visual Style**: Employs a cohesive `Card` where the header holds order-level data (Order Number, Date, Total) with a distinct background (`bg-muted/40`), and the body lists items separated by `Separator`s. Statuses are indicated with specific colored text and outlines of Lucide icons.
- **Interactivity**: Simple text search filtering across order number and item names. Includes action buttons (View Invoice, Archive) at the order level, and "Buy Again" at the item level.
