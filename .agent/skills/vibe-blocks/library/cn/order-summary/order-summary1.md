...
- **Objective**: Provide a comprehensive "Thank You / Order Confirmed" page that recaps the entire transaction.
- **Use Case**: Post-checkout confirmation screen or a standalone "View Order Details" page.
- **Visual Style**: Clean two-column layout on desktops. Features a prominent success message at the top with a green checkmark icon. The main content is split into left (items and price breakdown) and right (shipping address, method, and payment method) using clean, un-shadowed `Card` components.
- **Interactivity**: Static display component. Provides calls-to-action (CTAs) at the bottom for support, tracking, and continuing shopping.
