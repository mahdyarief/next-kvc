...
- **Objective**: Provide a compact, single-column order summary suitable for smaller viewports or simpler purchase flows.
- **Use Case**: Mobile-first eCommerce applications or straightforward checkout processes where a slim max-width is preferred.
- **Visual Style**: Clean, streamlined, single-column vertical flow. Uses minimal, un-shadowed Cards to group information linearly (Order Info, Items, Summary, Shipping & Payment).
- **Interactivity**: Static summary with primary "Track Order", "Receipt", and "Print" buttons stacked cleanly at the bottom.
