...
- **Objective**: Display a minimalist product detail page with a vertical image list, thumbnail navigation, and a focus on material textures.
- **Use Case**: Boutique leather goods, jewelry, or high-end fashion with a "lookbook" feel.
- **Visual Style**: Clean, high-fashion aesthetic. Uses "Instrument Sans" for a custom typographic feel. Gray muted background (`bg-muted`), thin separators, and uppercase headings.
- **Interactivity**: 
    - Vertical thumbnail nav: A sticky left sidebar with small square thumbnails that sync with the main scroll position.
    - Synchronized scrolling: Main images are displayed in a long vertical list (on desktop); clicking a thumbnail scrolls the page to relevant image.
    - Fullscreen zoom: Integrated `PhotoSwipe` with custom close and indicator controls.
    - Responsive transformation: Desktop uses a 3-column-like layout (thumbnails, images, details), while mobile collapses into a swipeable carousel.
    - Color variant labels: Live text feedback for selected color variant (e.g., "Color: Muted Rose").
    - Categorized details: Multi-item accordion for Composition, Care, and Shipping.

## Description
Product Detail 4 is designed for high-end boutique stores. It prioritizes discovery via a vertical scrollable gallery that encourages users to observe every detail of the product. The layout is innovative yet intuitive, featuring a thumbnail-driven navigation system that stays static while the gallery moves. Typographic choices like "Instrument Sans" and uppercase headings provide an editorial, magazine-like feel. The technical stack includes `photoswipe` for deep-zoom capabilities and `useMediaQuery` for seamless responsive transitions.
