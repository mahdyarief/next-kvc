...
- **Objective**: Provide a premium product detail view with advanced gallery features, variant selection, and collapsible product information.
- **Use Case**: Luxury handbags, high-end apparel, or artisanal goods that require high-resolution visual inspection.
- **Visual Style**: Clean, serif-based typography (using "Assistant" via Google Fonts), spacious layout with a 65/35 split, and sophisticated UI elements like `Accordion` and `PhotoSwipe`.
- **Interactivity**: 
    - Fullscreen gallery: Uses `PhotoSwipe` for a pro-level image viewing experience with zoom, previous/next controls, and an image indicator.
    - Carousel-to-grid: Responsive gallery that acts as a swipeable carousel on mobile and a fixed grid on desktop.
    - Advanced Quantity Input: Custom button-and-input combination with min/max validation.
    - Variant Selection: `RadioGroup` based selection for color and material with "Out of Stock" visual indicators (line-through and dimmed border).
    - Information Accordion: Collapsible sections for Composition, Delivery, Dimensions, and Care.
    - Floating Sidebar: The 35% detail column stays `sticky` while scrolling through the long image gallery on desktop.

## Description
Product Detail 2 is a high-performance, premium detail page template. It sets a new standard for e-commerce visual fidelity by integrating `PhotoSwipe` for immersive image exploration. The layout is optimized for high-ticket items, where detailed specifications and tactile descriptions are secondary to high-resolution visual confirmation. It includes custom form logic with `zod` for variant management and a dedicated care/delivery accordion system. The font choice and spaciousness evoke a boutique, architectural aesthetic.
