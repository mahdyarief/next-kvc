...
- **Objective**: Display a primary product detail layout with summary images, selection options, and key specifications.
- **Use Case**: Core product page for apparel or fashion items.
- **Visual Style**: Clean, grid-based layout with a gallery on the left and information panel on the right. High contrast typography and subtle use of icons (`CircleCheck`, `Star`).
- **Interactivity**: 
    - Carousel-based image gallery: Grid on desktop, swipeable carousel on mobile.
    - Size selection: `RadioGroup` with custom `SizeOption` buttons, including an "out of stock" diagonal line effect.
    - Rating system: Dynamic star renderer supporting full, half, and empty stars.
    - Key specs table: Responsive list of product attributes (Material, Style, Origin, etc.).

## Description
Product Detail 1 provides a comprehensive and accessible layout for single-product storytelling. It includes a sophisticated star rating component, a flexible price display that handles sales, and a robust size selection form. The component is fully responsive, transitioning from a split grid on large screens to a stacked single column with a mobile-optimized carousel on smaller viewports. It focus on conversion with a clear "Buy Now" CTA.
