...
- **Objective**: Implement a high-conversion product detail page for premium skin care, featuring a sticky sub-form that reveals on scroll and integrated "Related Items" discovery.
- **Use Case**: Cosmetics, apothecaries, or high-end lifestyle products requiring extensive information delivery and cross-selling.
- **Visual Style**: Modern, clean, and clinical aesthetic using "DM Sans". Large product imagery, rounded corners (`rounded-3xl`), and a focus on clarity through ample whitespace and subtle grays.
- **Interactivity**: 
    - Sticky "Quick Add" Bar: A fixed bottom bar containing product summary, variant selection, and "Add to Cart" button that appears automatically once the main CTA scrolls out of view.
    - Floating Gallery Navigation: Centered floating toolbar for `PhotoSwipe` with next/prev and close actions.
    - Image Hover Reveal: A `ZoomIn` icon badge appears on image hover to signal deep-zoom capability.
    - Integrated Recommendations: A horizontal carousel of "Related Items" built directly into the product info sidebar.
    - Custom Quantity Stepper: A minimalist +/- input with manual entry support and overflow protection.
    - Size-Linked Pricing: Real-time price updates (including sale calculations) when switching between volume variants (e.g., 100ml vs 200ml).
    - Multi-Source Info sections: An accordion system supporting both long-form text and unordered lists for ingredients or usage tips.

## Description
Product Detail 6 is a conversion-optimized template designed for the wellness and beauty sector. Its standout feature is the "Sticky Purchase Bar" which ensures the "Add to Cart" action is always within reach as the user scrolls through deep technical info or ingredient lists. The design uses `rounded-3xl` for a soft, approachable feel, while the "DM Sans" typography maintains a professional edge. Cross-selling is prioritized through an inline "Related Items" carousel, positioned strategically above the fold. Functional enhancements like `IntersectionObserver` drive the sticky bar logic, while `PhotoSwipe` provides a premium viewing experience.
    ...
    <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1">
      ...
      {size !== "sm" && sale && (
        <div
          data-sale={!!sale}
          aria-label="sale price"
          className="text-xl leading-normal font-semibold text-red-600"
        >
          {formatCurrency(sale, currency)}
        </div>
      )}
