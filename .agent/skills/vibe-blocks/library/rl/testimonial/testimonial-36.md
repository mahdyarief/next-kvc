...
export type Testimonial36Props = React.ComponentPropsWithoutRef<"section"> & Partial<Props>;
        ...
        <div className="gid-cols-1 grid gap-6 sm:grid-rows-6 md:grid-cols-2 md:gap-8 lg:grid-cols-4 lg:grid-rows-2">
          {testimonialCards.map((card, index) => {
            if (card.component === "background") {
              ...
              logoCardCount++;
              return <LogoCard key={index} isThird={logoCardCount === 3} {...card.props} />;
            } else {
...
const BackgroundImageCard = ({
  ...
  <div className={clsx("relative p-6 md:p-8 lg:p-6", { "order-last lg:order-none": isFourth })}>
...
const LogoCard = ({ logo, isThird }: LogoCardProps & { isThird?: boolean }) => (
 ...
 className={clsx(
      ...
      {
        "sm:col-start-2 sm:col-end-3 sm:row-start-4 sm:row-end-5 lg:col-auto lg:row-auto": isThird,
      },
    )}
>
...
Testimonial36.displayName = 'Testimonial36';
```
