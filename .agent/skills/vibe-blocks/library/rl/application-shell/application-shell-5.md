...
- **Objective**: Split layout with top navigation, left-side secondary panel, and main content area for complex tool layouts
- **Use Case**: Perfect for professional tools, creative applications, and complex dashboards requiring persistent access to secondary controls or local navigation while viewing main content. Ideal for scenarios where users need to switch between different toolsets or views frequently.
- **Visual Style**: Three-panel layout with top navigation bar, collapsible left-side secondary panel for tools or local navigation, and main content area. Clear visual hierarchy with distinct background colors and borders separating each zone. Panel proportions favor the main content area while keeping secondary controls accessible.
- **Interactivity**: Top-level navigation with dropdown menus, collapsible left-side panel with smooth transitions, local contextual controls within the secondary panel, responsive design that stacks panels vertically on mobile, click-outside behavior for dropdowns

## Description
A sophisticated split-layout application shell designed for professional tools and complex dashboards. Features a three-panel architecture with top navigation for global controls, a left-side secondary panel for contextual tools or local navigation, and a spacious main content area. The left panel provides persistent access to secondary controls while keeping the main content area focused and uncluttered. Built for applications requiring frequent switching between different toolsets or views. The responsive design intelligently collapses and stacks panels on mobile devices while maintaining full functionality. Includes smooth transitions for panel collapse/expand states and comprehensive dropdown menus in the top navigation.
...
export type ApplicationShell5Props = React.ComponentPropsWithoutRef<"section"> & Partial<Props>;

export const ApplicationShell5 = (props: ApplicationShell5Props) => {
  const { logo, navLinks } = {
    ...ApplicationShell5Defaults,
    ...props,
  };
...
>
      ...
      <main className="bg-background-secondary">
        ...
        <div className="container grid grid-cols-1 gap-12 px-6 py-8 md:grid-cols-[0.5fr_1fr] md:py-10 lg:px-0 lg:py-12">
...
export const ApplicationShell5Defaults: Props = {
  logo: {
    url: "#",
...
ApplicationShell5.displayName = 'ApplicationShell5';
```
