...
- **Objective**: Top navigation bar with animated hamburger menu, integrated search, notification center, and user profile dropdown
- **Use Case**: Perfect for marketing websites, blogs, and content sites where users expect traditional website navigation. Suitable for audiences unfamiliar with sidebar patterns. Ideal for public-facing sites requiring search and user account features.
- **Visual Style**: Clean horizontal navigation bar with logo left-aligned, navigation links center, utility icons right-aligned. Mobile hamburger menu transforms to X animation. Sticky positioning with border separation.
- **Interactivity**: Hamburger menu with framer-motion animation (transforms to X), mobile search overlay with backdrop click-to-close, notification dropdown with live badge counter, user profile dropdown, responsive breakpoint at 992px, click-outside behavior for all overlays

## Description
A production-ready top navigation shell featuring smooth hamburger-to-X animations powered by framer-motion, intelligent mobile search overlay with click-outside detection, and comprehensive dropdown menus. Includes a notification center with unread badges, user profile management, and responsive mobile menu. Built with performance in mind using framer-motion for 60fps animations, proper event delegation for click-outside behavior, and memoized components. Supports nested navigation with dropdown menus and provides placeholder regions for page header and main content.
...
export type ApplicationShell1Props = React.ComponentPropsWithoutRef<"section"> & Partial<Props>;
