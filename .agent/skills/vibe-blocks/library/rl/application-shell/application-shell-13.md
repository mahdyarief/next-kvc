...
- **Objective**: Sticky left-side panel for persistent sub-navigation below top navigation bar, ideal for documentation and settings interfaces
- **Use Case**: Perfect for documentation sites, settings pages, and long-form content where users need persistent access to section navigation while scrolling through content. Ideal for technical documentation, help centers, configuration pages, and any interface where users benefit from sticky sub-navigation that tracks scroll position and highlights current sections. Best for scenarios where users need to jump between sections quickly without losing their place in the main content.
- **Visual Style**: Clean top navigation bar paired with a sticky left-side panel that remains visible during vertical scrolling. The secondary panel features section links with active state indicators, smooth scrolling behavior, and clear visual hierarchy. Distinct background colors separate the top nav, sticky left panel, and main content area. The design emphasizes content discoverability and easy navigation through long documents or complex settings pages.
- **Interactivity**: Sticky left panel behavior that remains fixed during vertical scrolling with active section tracking, smooth scroll-to-section animations, click-to-jump navigation between sections, visual highlighting of current section, responsive design that collapses the panel on mobile devices, click-outside behavior for any dropdowns in the top navigation, and synchronized scroll position tracking that updates the active section indicator as users scroll through content

## Description
A specialized documentation and settings layout featuring a sticky left-side panel for persistent sub-navigation below a top navigation bar. This design excels in documentation sites, help centers, and settings pages where users need constant access to section navigation while scrolling through lengthy content. The sticky left panel remains fixed during vertical scrolling and actively tracks the user's scroll position, highlighting the current section and enabling quick jumps between topics. Built for technical documentation, configuration interfaces, and any content-heavy application where section-based navigation improves usability. The responsive design intelligently collapses the sticky panel on mobile devices while maintaining scroll tracking and smooth scroll-to-section animations in the main content area.
...
export type ApplicationShell13Props = React.ComponentPropsWithoutRef<"section"> & Partial<Props>;

export const ApplicationShell13 = (props: ApplicationShell13Props) => {
  const { logo, navLinks } = {
    ...ApplicationShell13Defaults,
    ...props,
  };
...
>
      ...
      <main className="relative flex min-h-screen flex-col lg:flex-row">
        <div className="static top-18 order-last flex flex-col lg:sticky lg:order-first lg:h-[calc(100vh-4.5rem)] lg:overflow-auto lg:border-r lg:border-[#d3d3d3]">
...
export const ApplicationShell13Defaults: Props = {
  logo: {
    url: "#",
...
ApplicationShell13.displayName = 'ApplicationShell13';
```
