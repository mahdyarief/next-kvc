...
export type Links11Props = React.ComponentPropsWithoutRef<"section"> & Partial<Props>;

export const Links11 = (props: Links11Props) => {
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        {category.links.map((link, index) => (
          <Button
            key={index}
 className={clsx({
              "flex-col gap-0 p-0": link.type === CategoryLinkType.Card,
              "col-span-2 sm:col-span-3": link.type === CategoryLinkType.Button,
            })}
            {...link.button}
            asChild
>
 ...
 className="aspect-square size-full object-cover"
                  ...
                  <span className="mt-2 flex flex-col gap-y-1 whitespace-normal text-center">
                    <span>{link.button.title}</span>
                    <span className="font-semibold md:text-md">{link.button.price}</span>
                  </span>
                </React.Fragment>
              )}
            </a>
...
export const Links11Defaults: Props = {
  author: {
    avatar: {
