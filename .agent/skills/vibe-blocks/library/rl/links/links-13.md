...
export type Links13Props = React.ComponentPropsWithoutRef<"section"> & Partial<Props>;

export const Links13 = (props: Links13Props) => {
    ...
    <div className="flex flex-col gap-4 text-center">
      ...
      {category.links.map((link, index) => (
        <a
          href={link.url}
          key={index}
 className="flex items-center gap-x-2 border border-border-primary p-2"
>
          <div>
            <img src={link.image.src} alt={link.image.alt} className="size-16 object-cover" />
          </div>
          <div className="grow pr-18 text-center">
            <p>{link.title}</p>
          </div>
        </a>
      ))}
...
export const Links13Defaults: Props = {
  author: {
    avatar: {
