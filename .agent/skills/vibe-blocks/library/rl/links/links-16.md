...
export type Links16Props = React.ComponentPropsWithoutRef<"section"> & Partial<Props>;

export const Links16 = (props: Links16Props) => {
    ...
    <div className="flex flex-col gap-4 md:text-center">
      ...
      {category.links.map((link, index) => (
        <a
          href={link.url}
          key={index}
 className="flex items-center gap-x-4 border border-border-primary p-3"
>
          <div>
            <img src={link.image?.src} alt={link.image?.alt} className="size-8" />
          </div>
          <p>{link.title}</p>
        </a>
      ))}
...
export const Links16Defaults: Props = {
  author: {
    avatar: {
