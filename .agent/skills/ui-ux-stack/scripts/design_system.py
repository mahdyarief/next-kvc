...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        return {
            ...
            "colors": {
                "primary": best_color.get("Primary", "primary"),
                "secondary": best_color.get("Secondary", "secondary"),
                "accent": best_color.get("Accent", "accent"),
                "background": best_color.get("Background", "background"),
                "foreground": best_color.get("Foreground", "foreground"),
                "muted": best_color.get("Muted", "muted"),
...
def format_ascii_box(design_system: dict) -> str:
    ...
    # Colors section
    lines.append("|  COLORS (Semantic Tokens):".ljust(BOX_WIDTH) + "|")
    lines.append(f"|     Primary:    {colors.get('primary', '')}".ljust(BOX_WIDTH) + "|")
    lines.append(f"|     Secondary:  {colors.get('secondary', '')}".ljust(BOX_WIDTH) + "|")
    lines.append(f"|     Accent:     {colors.get('accent', '')}".ljust(BOX_WIDTH) + "|")
    lines.append(f"|     Background: {colors.get('background', '')}".ljust(BOX_WIDTH) + "|")
    lines.append(f"|     Foreground: {colors.get('foreground', '')}".ljust(BOX_WIDTH) + "|")
    lines.append(f"|     Muted:      {colors.get('muted', '')}".ljust(BOX_WIDTH) + "|")
    if colors.get("notes"):
...
def format_markdown(design_system: dict) -> str:
    ...
    # Colors section
    lines.append("### Colors (Semantic Tokens)")
    lines.append(f"| Role | Token |")
    lines.append(f"|------|-------|")
    ...
    lines.append(f"| Foreground | `{colors.get('foreground', '')}` |")
    lines.append(f"| Muted | `{colors.get('muted', '')}` |")
    if colors.get("notes"):
