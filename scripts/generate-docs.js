...
function generateExample(schema, root, depth = 0) {
  ...
  switch (schema.type) {
    ...
    case "boolean":
      return true;
    case "object":
      return { text: "Hello from NEXT-KVC!" };
    default:
      return null;
  }
}
...
let md = "";
md += `# NEXT-KVC API Documentation\n\n`;
md += `# Advanced Dashboard Starter — Complete API Reference\n\n`;
md += `Professional Dashboard Starter REST API with **${Object.keys(swagger.paths).length} routes** for comprehensive application management.\n\n`;
...
md += `| :--- | :--- | :--- |\n`;
md += `| \`userId\` | System User identifier (CUID) | \`clx123abc\` |\n`;
md += `| \`role\` | System access level | \`ADMIN\` |\n\n`;
md += `---\n\n`;
...
for (const [tag, endpoints] of Object.entries(tagMap)) {
  ...
  for (const { path, method, spec } of endpoints) {
    ...
    if (responseCodes.length > 0) {
      ...
      if (resp200?.content) {
        for (const [_ct, ctSpec] of Object.entries(resp200.content)) {
